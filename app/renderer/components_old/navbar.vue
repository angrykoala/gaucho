            ...
            <nav class="nav-extended">
                <div class="nav-wrapper">
                    ...
                    <ul class="right navbar-buttons">
                        <li><a id="tap-edit" :class="{'edit-button-active': editMode}" class="edit-button" @click="toggleEdit"><i class="material-icons unselectable-text">mode_edit</i></a></li>
                        <li><a id="navbar-menu-button" :href="'#tab0'" data-activates="navbar-menu" data-gutter="30"><i class="material-icons small unselectable-text">menu</i></a></li>
                    </ul>
                        ...
                        <ul id="navbar-tabs" class="tabs tabs-transparent">
                            <template v-for="(suite,index) in suites">
                                <li class="tab col s3 unselectable-text" @dragover="dragOver(index)">
                                    <a :href="'#tab'+index" :class="{ active: index===0 }" draggable="false" class="tab-button" @click="onTabSelected(index)">
                                        <template v-if="editMode && index===activeSuite">
                                            <input id="suite-title-input" v-model="suite.title" type="text" class="validate tab-text">
                                        </template>
                                        <span v-show="!editMode || index!==activeSuite" class="tab-text">{{suite.title}}</span>
                                    </a>
                                </li>
                            </template>
                        </ul>
